...
そもそもWayback Machineがリダイレクトを記録できた理由として最も考えられるのは、対象サイトのDNSがS3バケット（Amazon）やR2バケット（Cloudflare）といったクラウドストレージに向いていたからだ。Waybackのクローラーがサイトを訪れた際、mvjfkakfkfkaiai[.]comへと転送されたわけだ。<br>
リダイレクトが消えた理由としては、そのバケットが削除されたと考えるのが自然だろう。
...
さらに推測できることとして、dkaksdaksortor[.]comはただの使い捨ての窓口に過ぎず、フィッシングや悪意ある広告を通じてばら撒かれ、被害者を本命のインフラへ誘導するための踏み台だったと考えられる。実際の悪意あるJavaScriptはmvjfkakfkfkaiai[.]comに置かれており、そこからwindowwashingexpert[.]com上のPHPウェブシェルを経由して偽CloudflareのCAPTCHAが読み込まれる仕組みだった。
